...
AFW_WWA_BASE_URL = "https://bifrost.afweather.mil/ewa/wwa/v1"
AFW_WWA_PLATFORM_ID = "KMEM"
AFW_WWA_TIMEOUT_SECONDS = 25
AFW_LIGHTNING_DISPLAY_WINDOW_MINUTES = 60
AFW_LIGHTNING_CLOSURE_DISTANCE_NM = 5.0
...
def format_afw_time_z(value):
    dt = parse_z_datetime(value)
    if not dt:
        return "--"
...
def afw_http_get_json(url):
    request = urllib.request.Request(
        ...
        headers={
            "User-Agent": "Mozilla/5.0",
            "Accept": "application/json, text/plain, */*",
            "Referer": "https://bifrost.afweather.mil/"
        }
    )
...
def afw_wwa_is_lightning(item):
    text = json.dumps(item or {}, ensure_ascii=False).upper()
    return "LIGHTNING" in text or "LIGHTNING_DISTANCE" in text or "LTG" in text


def afw_wwa_is_lightning_within_5_warning(item):
    """
    Auto flight-line closure is controlled by the live AFW/BIFROST WWA product,
    not by raw strike distance alone.

    True only for active/open WARNING products that are clearly for lightning
    at/within 5 NM.
    """
    if not isinstance(item, dict):
        return False

    if str(item.get("wwaType") or "").upper() != "WARNING":
        return False

    if not afw_wwa_is_lightning(item):
        return False

    text = json.dumps(item, ensure_ascii=False).upper()
    text = text.replace("NAUTICAL MILES", "NM")
    text = text.replace("NAUTICAL MILE", "NM")

    if "LIGHTNING_DISTANCE" in text:
        return True

    if ("LIGHTNING" in text or "LTG" in text) and re.search(r"(WITHIN|LESS THAN OR EQUAL TO|<=|≤)\s*5\s*NM", text):
        return True

    if ("LIGHTNING" in text or "LTG" in text) and re.search(r"5\s*NM", text):
        return True

    return False


def afw_wwa_first_criteria(item):
    issued = item.get("issuedCriteria") if isinstance(item, dict) else None
    if isinstance(issued, list) and issued:
        first = issued[0] if isinstance(issued[0], dict) else {}
        criteria = first.get("criteria") if isinstance(first.get("criteria"), dict) else {}
        return first, criteria
    return {}, {}


def afw_wwa_compact_label(item):
    wwa_type = str(item.get("wwaType") or "WWA").upper()
    message = str(item.get("message") or item.get("wwaText") or "").strip()
    first, criteria = afw_wwa_first_criteria(item)
    group = str(criteria.get("group") or "").upper()
    severity = str(criteria.get("severity") or "").upper()
    criteria_text = str(criteria.get("text") or "").upper()
    combined = f"{group} {severity} {criteria_text} {message}".upper()

    if "LIGHTNING" in combined or "LIGHTNING_DISTANCE" in combined:
        base = "LTG ≤5NM"
    elif "THUNDER" in combined or "TSTM" in combined:
        if "MODERATE" in combined or severity == "MODERATE":
            base = "MOD TSTMS"
        elif "SEVERE" in combined or severity == "SEVERE":
            base = "SEV TSTMS"
        else:
            base = "TSTMS"
    elif group:
        base = group.replace("THUNDERSTORMS", "TSTMS")
    else:
        clean = re.sub(r"\s+", " ", message).strip()
        base = clean[:34].rstrip() if clean else "WWA"

    return f"{base} {wwa_type}".strip()


def afw_wwa_time_range(item):
    first, _criteria = afw_wwa_first_criteria(item)
    start = first.get("startTime") or item.get("startTime") or item.get("issueTime")
    end = first.get("endTime") or item.get("endTime")

    if start and end:
        return f"{format_afw_time_z(start)}-{format_afw_time_z(end)}"
    if start:
        return f"FM {format_afw_time_z(start)}"
    return ""


def normalize_afw_wwa_item(item):
    if not isinstance(item, dict):
        return None

    wwa_type = str(item.get("wwaType") or "WWA").upper()
    dot = "🔴" if wwa_type == "WARNING" else "🟡" if wwa_type == "WATCH" else "🟠"
    compact = afw_wwa_compact_label(item)
    time_range = afw_wwa_time_range(item)
    display = f"{dot} {compact}" + (f" {time_range}" if time_range else "")

    return {
        "id": item.get("_id") or "",
        "iwwcId": item.get("iwwcId") or "",
        "platformId": item.get("platformId") or AFW_WWA_PLATFORM_ID,
        "wwaType": wwa_type,
        "status": item.get("status") or "",
        "issuanceState": item.get("issuanceState") or "",
        "issueTime": item.get("issueTime") or "",
        "message": item.get("message") or "",
        "wwaText": item.get("wwaText") or "",
        "displayText": display,
        "compactLabel": compact,
        "timeRange": time_range,
        "isLightning": afw_wwa_is_lightning(item),
        "raw": item
    }


def afw_wwa_active_display_lines(items):
    lines = []
    seen = set()

    priority = {"WARNING": 0, "WATCH": 1, "ADVISORY": 2}
    ordered = sorted(items, key=lambda x: (priority.get(x.get("wwaType", ""), 9), x.get("displayText", "")))

    for item in ordered:
        text = item.get("displayText") or ""
...
def afw_fetch_active_wwas(now_z):
    valid_time = now_z.strftime("%Y-%m-%dT%H:%M:%SZ")
    query = urllib.parse.urlencode([
        ("platform_id", AFW_WWA_PLATFORM_ID),
        ("wwa_status", "OPEN"),
        ("wwa_status", "AMENDED"),
        ("valid_time", valid_time)
    ])
    url = f"{AFW_WWA_BASE_URL}/wwas?{query}"
    raw = afw_http_get_json(url)
    ...
    items = []
    for item in raw:
        normalized = normalize_afw_wwa_item(item)
        if normalized:
...
def afw_flatten_verification_options(raw):
    out = []
...
def afw_fetch_verification_options(item):
    wwa_type = str(item.get("wwaType") or "").upper()
    iwwc_id = str(item.get("iwwcId") or "").strip()

    if not wwa_type or not iwwc_id:
        return []

    url = f"{AFW_WWA_BASE_URL}/sites/{urllib.parse.quote(AFW_WWA_PLATFORM_ID)}/wwas/{urllib.parse.quote(wwa_type)}/{urllib.parse.quote(iwwc_id)}/verification-options"
    raw = afw_http_get_json(url)
    return afw_flatten_verification_options(raw)


def afw_parse_lightning_distance(option):
    distance = option.get("observationDistance") if isinstance(option, dict) else None

    if isinstance(distance, dict):
        value = distance.get("value")
        unit = str(distance.get("unit") or "NM").upper()
        try:
            value = float(value)
        except Exception:
            value = None
    ...
    phenoms = option.get("observedPhenoms") if isinstance(option, dict) else None
    if isinstance(phenoms, dict):
        raw = str(phenoms.get("LIGHTNING_DISTANCE") or "")
        m = re.search(r"([0-9]+(?:\.[0-9]+)?)\s*NM", raw.upper())
        if m:
            return float(m.group(1))

    raw_report = str(option.get("rawReport") or "") if isinstance(option, dict) else ""
    m = re.search(r"Distance:\s*\[value:\s*([0-9]+(?:\.[0-9]+)?)\s*,\s*unit:\s*NM", raw_report, re.I)
    if m:
        return float(m.group(1))
...
def afw_lightning_option_to_strike(option, now_z):
    if not isinstance(option, dict):
        return None

    text = json.dumps(option, ensure_ascii=False).upper()
    if "LIGHTNING_DISTANCE" not in text and "GLOBAL_LIGHTNING_NETWORK" not in text and "LIGHTNING" not in text:
        return None

    distance_nm = afw_parse_lightning_distance(option)
    observed_time = parse_z_datetime(option.get("observationTime"))

    if distance_nm is None or not observed_time:
        return None

    age_minutes = int(max(0, round((now_z - observed_time).total_seconds() / 60.0)))

    return {
        "distanceNm": round(distance_nm, 1 if distance_nm >= 10 else 2),
        "observationTimeZ": observed_time.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "ageMinutes": age_minutes,
        "within5Nm": distance_nm <= AFW_LIGHTNING_CLOSURE_DISTANCE_NM,
        "optionType": option.get("optionType") or "",
        "matchedCriteriaCompletely": bool(option.get("matchedCriteriaCompletely")),
        "rawReport": option.get("rawReport") or ""
    }


def afw_previous_strike_if_recent(previous_data, now_z):
    previous = (previous_data or {}).get("afwLightningLastStrike")

    if not isinstance(previous, dict):
        return None

    observed = parse_z_datetime(previous.get("observationTimeZ"))
    if not observed:
        return None

    age = int(max(0, round((now_z - observed).total_seconds() / 60.0)))
    if age > AFW_LIGHTNING_DISPLAY_WINDOW_MINUTES:
        return None

    try:
        dist = float(previous.get("distanceNm"))
    except Exception:
        return None

    out = dict(previous)
    out["distanceNm"] = round(dist, 1 if dist >= 10 else 2)
    out["ageMinutes"] = age
    out["within5Nm"] = dist <= AFW_LIGHTNING_CLOSURE_DISTANCE_NM
    return out


def afw_strike_display(strike):
    if not strike:
        return ""

    try:
        dist = float(strike.get("distanceNm"))
    except Exception:
        return ""

    age = strike.get("ageMinutes")
    try:
        age = int(age)
    except Exception:
        age = 0

    dist_text = f"{dist:.1f}" if dist >= 10 else f"{dist:.2f}".rstrip("0").rstrip(".")
    return f"⚡ {dist_text} NM\n{age} MIN AGO"


def fetch_afw_wwa_and_lightning(previous_data, now_z):
    default = {
        "afwWwaFetchStatus": "NOT_CHECKED",
        "afwWwaUpdatedZ": now_z.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "afwWwaItems": [],
        "afwWwaDisplayLines": [],
        "afwWwaDisplayText": "",
        "afwWwaSourceUrl": "",
        ...
        "afwLightningDisplay": "",
        "afwLightningDisplayWindowMinutes": AFW_LIGHTNING_DISPLAY_WINDOW_MINUTES,
        "autoLightningClosureActive": False,
        "autoLightningClosureReason": "",
        "autoLightningClosureUpdatedZ": ""
    }

    try:
        items, url = afw_fetch_active_wwas(now_z)
        default["afwWwaFetchStatus"] = "OK"
        default["afwWwaSourceUrl"] = url
        default["afwWwaItems"] = items
        default["afwWwaDisplayLines"] = afw_wwa_active_display_lines(items)
        default["afwWwaDisplayText"] = "\n".join(default["afwWwaDisplayLines"])

        lightning_items = [item for item in items if item.get("isLightning")]
        strikes = []

        for item in lightning_items:
            try:
                options = afw_fetch_verification_options(item)
            except Exception as error:
                print(f"AFW WWA verification fetch failed for {item.get('wwaType')} {item.get('iwwcId')}: {error}")
                options = []

            for option in options:
                strike = afw_lightning_option_to_strike(option, now_z)
                if strike:
                    strike["wwaType"] = item.get("wwaType")
                    strike["iwwcId"] = item.get("iwwcId")
                    strikes.append(strike)

        strikes.sort(key=lambda x: parse_z_datetime(x.get("observationTimeZ")) or datetime.min.replace(tzinfo=timezone.utc), reverse=True)
        recent_strikes = [s for s in strikes if s.get("ageMinutes", 9999) <= AFW_LIGHTNING_DISPLAY_WINDOW_MINUTES]

        last_strike = recent_strikes[0] if recent_strikes else afw_previous_strike_if_recent(previous_data, now_z)

        if last_strike:
            default["afwLightningLastStrike"] = last_strike
            default["afwLightningDisplay"] = afw_strike_display(last_strike)
        elif lightning_items:
            # Awareness only: active WWA lightning product but no verified recent strike to display.
            # No colored dot emoji here; dots belong in the WWA lines below TAF.
            warning_active = any(item.get("wwaType") == "WARNING" for item in lightning_items)
            default["afwLightningDisplay"] = "WWA LTG WARNING" if warning_active else "WWA LTG WATCH"

        active_lightning_within_5_warning = any(
            afw_wwa_is_lightning_within_5_warning(item)
            for item in items
        )

        # Automatic closure follows only the live/open AFW lightning-within-5NM WARNING.
        # Raw strike distance by itself does not open or close the automatic banner.
        # The 60-minute strike display is awareness only and can outlive the warning.
        if active_lightning_within_5_warning:
            default["autoLightningClosureActive"] = True
            default["autoLightningClosureReason"] = "AFW_ACTIVE_LIGHTNING_WITHIN_5_WARNING"
            default["autoLightningClosureUpdatedZ"] = now_z.strftime("%Y-%m-%dT%H:%M:%SZ")

        return default

    except Exception as error:
        print("AFW WWA: fetch failed:", error)
        default["afwWwaFetchStatus"] = "FAILED"
        # Keep last strike awareness up to 60 minutes, but do not keep/guess auto closure.
        last_strike = afw_previous_strike_if_recent(previous_data, now_z)
        if last_strike:
            default["afwLightningLastStrike"] = last_strike
            default["afwLightningDisplay"] = afw_strike_display(last_strike)
        return default
...
def build_weather_json():
    ...
    afw_wwa_data = fetch_afw_wwa_and_lightning(previous_data, now_z)

    # Prefer verified KMEM AFW/BIFROST lightning strike display in LIGHTNING box for up to 60 minutes.
    if afw_wwa_data.get("afwLightningDisplay"):
        last_strike = afw_wwa_data.get("afwLightningLastStrike") or {}
        strike_within_5 = bool(last_strike.get("within5Nm"))
        try:
            strike_age = int(last_strike.get("ageMinutes"))
        except Exception:
            strike_age = 9999

        # Red only when the displayed strike is within 5 NM and less than 30 minutes old.
        # Yellow when the latest strike is outside 5 NM, or when it is 30+ minutes old.
        strike_red = strike_within_5 and strike_age < 30

        lightning_summary = {
            "lightning": afw_wwa_data["afwLightningDisplay"],
            "lightningSeverity": "close" if strike_red else "watch",
            "lightningTone": "red" if strike_red else "yellow",
            "lightningFlash": strike_red,
            "lightningPulse": not strike_red,
            "lightningSource": "AFW_BIFROST",
            "lightningLogText": f"AFW/BIFROST KMEM lightning verification: {afw_wwa_data['afwLightningDisplay']}"
        }
    ...
    data = {
        ...
        "afwWwaFetchStatus": afw_wwa_data["afwWwaFetchStatus"],
        "afwWwaUpdatedZ": afw_wwa_data["afwWwaUpdatedZ"],
        "afwWwaItems": afw_wwa_data["afwWwaItems"],
        "afwWwaDisplayLines": afw_wwa_data["afwWwaDisplayLines"],
        "afwWwaDisplayText": afw_wwa_data["afwWwaDisplayText"],
        "afwWwaSourceUrl": afw_wwa_data["afwWwaSourceUrl"],
        "afwLightningLastStrike": afw_wwa_data["afwLightningLastStrike"],
        "afwLightningDisplay": afw_wwa_data["afwLightningDisplay"],
        "afwLightningDisplayWindowMinutes": afw_wwa_data["afwLightningDisplayWindowMinutes"],
        "autoLightningClosureActive": afw_wwa_data["autoLightningClosureActive"],
        "autoLightningClosureReason": afw_wwa_data["autoLightningClosureReason"],
        "autoLightningClosureUpdatedZ": afw_wwa_data["autoLightningClosureUpdatedZ"],
    ...
    print("AFW WWA:", data["afwWwaFetchStatus"], data.get("afwWwaDisplayText") or "NONE")
    print("AFW LIGHTNING:", data.get("afwLightningDisplay") or "NONE", "AUTO CLOSURE:", data.get("autoLightningClosureActive"))
