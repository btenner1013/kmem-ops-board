            ...
            with self.subTest(expected_transport=expected_transport):
                with (
                    ...
                    mock.patch.object(
                        nms,
                        "python_runtime_http_request",
                        return_value=b"python runtime response",
                    ) as python_runtime,
                    mock.patch("builtins.print") as output,
                ):
    ...
    def test_python_runtime_https_uses_system_route_and_verified_tls_without_nested_launcher(self):
        ...
        with (
            ...
            result = nms.python_runtime_http_request(
                "GET",
                "https://api-staging.cgifederal-aim.com/nmsapi/v1/notams?location=KMEM",
                {"Authorization": authorization},
            )
    ...
    def test_curl_failure_uses_powershell_without_second_python_executable(self):
        ...
        with (
            ...
            mock.patch.object(
                nms,
                "curl_http_request",
                side_effect=nms.NmsCompatibilityError("curl exit 2"),
            ),
            mock.patch.object(
                nms,
                "powershell_http_request",
