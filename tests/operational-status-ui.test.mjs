...
test("ATIS runway closures remain separate from NMS closure records and expose source differences", () => {
  const provenanceContext = vm.createContext({
    atisDisplayState: data => ({ mode: data?.atisSourceIsCurrent === true ? "current" : "last-reported" }),
  });
  vm.runInContext(
    "function notamFullTextFromItem(item){return String(item?.rawText||item?.text||item?.displayText||\"\");}\n" +
      sourceBetween("function canonicalRunwayDesignator", "function atisClosureSectionHtml") +
      "globalThis.currentAtisClosureState=currentAtisClosureState;" +
      "globalThis.runwayClosureProvenance=runwayClosureProvenance;",
    provenanceContext,
  );

  const currentAtis = {
    atisSourceIsCurrent: true,
    atisReportedLetter: "N",
    atisReportedClosedRunways: "18R / 36L / 9 / 27",
  };
  const withoutNmsClosures = provenanceContext.runwayClosureProvenance(currentAtis, [], "OK");
  assert.equal(withoutNmsClosures.atis.display, "18R / 36L / 9 / 27");
  assert.deepEqual([...withoutNmsClosures.nmsRunways], []);
  assert.equal(withoutNmsClosures.differs, true);

  const matchingNmsClosures = provenanceContext.runwayClosureProvenance(
    currentAtis,
    [
  ...
  assert.deepEqual([...matchingNmsClosures.nmsRunways], ["18R", "36L", "9", "27"]);
  assert.equal(matchingNmsClosures.differs, false);

  const nmsOnlyClosure = provenanceContext.runwayClosureProvenance(
    { ...currentAtis, atisReportedClosedRunways: "NONE" },
    [{ text: "RWY 09/27 CLSD" }],
    "OK",
  );
  assert.equal(nmsOnlyClosure.atis.display, "NONE");
  assert.equal(nmsOnlyClosure.differs, true);

  const unavailableNms = provenanceContext.runwayClosureProvenance(currentAtis, [], "ERROR");
  assert.equal(unavailableNms.differs, false, "a failed NMS feed is not presented as a comparable source");

  const staleAtis = provenanceContext.runwayClosureProvenance(
    { ...currentAtis, atisSourceIsCurrent: false },
    [],
    "OK",
  );
  assert.equal(staleAtis.atis.display, "NOT CURRENT");
  assert.equal(staleAtis.differs, false);

  const displaySource = sourceBetween("function setMilNotamsDisplay", "function measureMilNotamScroll");
  assert.match(displaySource, /NMS:.*RWY-CLOSURE RECORD/);
  assert.match(displaySource, /ATIS CLOSED/);
  assert.match(displaySource, /SOURCE DIFFERENCE/);
  assert.match(displaySource, /NMS RWY-CLOSURE RECORDS NOW/);
  assert.doesNotMatch(displaySource, /notamSectionHtml\("RWY CLOSURES NOW"/);
  assert.match(
    sourceBetween("function currentAtisClosureState", "function runwayClosureProvenance"),
    /atisDisplayState\(data\|\|\{\}\)\.mode!=="current"/,
  );
});
